...
def generate_interpolation_video(network_pkl, truncation_psi, snapshot=None, grid_size=[1,1], image_zoom=1, duration_sec=30.0, smoothing_sec=1.0, mp4_fps=30, mp4_codec='libx265', mp4_bitrate='16M', random_seed=1000, minibatch_size=8):
    ...
    Gs_kwargs.minibatch_size = minibatch_size
    ...
    noises = {var: random_state.randn(*var.shape.as_list()) for var in noise_vars} # [height, width]

    # Frame generation func for moviepy.
    def make_frame(t):
        for noise in noises:
            noise += random_state.randn(*noise.shape.as_list())
        tflib.set_vars(noises) # [height, width]

        frame_idx = int(np.clip(np.round(t * mp4_fps), 0, num_frames - 1))
        latents = all_latents[frame_idx]
        images = Gs.run(latents, None, **Gs_kwargs) # [minibatch, height, width, channel]
        grid = misc.create_image_grid(images, grid_size)
        if image_zoom > 1:
    ...
    moviepy.editor.VideoClip(make_frame, duration=duration_sec).write_videofile(dnnlib.make_run_dir_path('seed%04d.mp4' % random_seed), fps=mp4_fps, codec='libx264', bitrate=mp4_bitrate)
...
Run 'python %(prog)s <subcommand> --help' for subcommand help.''',
    ...
    parser_generate_images = subparsers.add_parser('generate-video', help='Generate interpolation video')
    parser_generate_images.add_argument('--network', help='Network pickle filename', dest='network_pkl', required=True)
    parser_generate_images.add_argument('--truncation-psi', type=float, help='Truncation psi (default: %(default)s)', default=0.5)
    parser_generate_images.add_argument('--result-dir', help='Root directory for run results (default: %(default)s)', default='results', metavar='DIR')
    ...
    func_name_map = {
        ...
        'generate-video': 'run_generator.generate_interpolation_video',
